...
export const createSegment = (getVectors, stimuli) => {
  ...
  const normals = bagOfWords.map(([...words]) =>
    sumAndNormalize(getVectors(words).map(({ values }) => values))
  );
...
const seriesToTensor = (series, getVectors, backlogMinutes) => {
  const segments = [...Array(backlogMinutes)].map((_, i) =>
    createSegment(getVectors, series[i])
  );
...
export const createLstm = async (getVectors, options = defaultOptions) => {
  const { learningRate, backlogMinutes, predictionMinutes } = {
    ...defaultOptions,
    ...options,
  };
  ...
  return {
    nextEpoch: async (data) => {
      ...
      if (numberOfSegments !== maxSegments) {
        throw new Error(
          `Expected ${maxSegments} segments, encountered ${numberOfSegments}.`
        );
      }
