...
export const toSingleLine = text => text
  .trim()
  .toLowerCase()
...
export const toWords = singleLine => singleLine
  .split(' ')
  .filter(e => (!!e && e.length > 0));

export const createClient = options => Promise
  .resolve(new Twitter(options))
  .then(
    client => ({
      subscribeTo: (track, callback) => client
        .stream('statuses/filter', { track })
        .on('data', obj => typeCheck("{text:String,...}", obj) && callback(obj)),
    }),
  );
