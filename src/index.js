...
const forceMinutes = (t) => moment(t).startOf("minute").toDate().getTime();
...
(async () => {
  ...
  const {
    push: pushPrice,
    get: getPrice,
    discard: discardPrice,
  } = await createBacklog(false, "(Number, Number, Number, Number)");
  const {
    push: pushMerge,
    get: getMerge,
    discard: discardMerge,
  } = await createBacklog(
    false,
    "{prices:(Number,Number,Number,Number),words:[[String]]}"
  );
  const {
    push: pushWords,
    get: getWords,
    discard: discardWords,
  } = await createBacklog(true, "[String]");
  ...
  await createSync("BTC", async (values, meta) => {
    values.map(([t, ...ohlc]) =>
      pushPrice(
    ...
    Object.entries(
      mergeBacklogs({ prices: getPrice, words: getWords })
    ...
    if (max >= min + dt) {
      ...
      const future = getBetween(
        getMerge,
        max - backlogMinutes * 60 * 1000,
        max
      );
